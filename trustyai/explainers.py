...
class CounterfactualResult(ExplanationVisualiser):
    """Wraps Counterfactual results. This object is returned by the :class:`~CounterfactualExplainer`,
    and provides a variety of methods to visualize and interact with the results of the counterfactual explanation.
    """
...
class CounterfactualExplainer:
    ...
    of nearby inputs that produces the desired outcome. Mathematically, if we have a model :math:`f`, some input
    :math:`x` and a desired model output :math:`y'`, the counterfactual explainer finds some nearby
    input :math:`x'` such that :math:`f(x') = y'`.
    """
    ...
    def explain(
        ...
        """Request for a counterfactual explanation given a :class:`~CounterfactualPrediction` and a :class:`~PredictionProvider`

        Parameters
        ----------
        prediction : :obj:`trustyai.model.CounterfactualPrediction`
            The counterfactual prediction as returned by :func:`~trustyai.model.counterfactual_prediction`. This object
            wraps both the initial input and desired output together.
        model : :obj:`~trustyai.model.PredictionProvider`
            The TrustyAI PredictionProvider, as generated by :class:`~trustyai.model.Model` or :class:`~trustyai.model.ArrowModel`.
...
class LimeResults(ExplanationVisualiser):
    ...
    def as_dataframe(self) -> pd.DataFrame:
        ...
        -------
        pandas.DataFrame
            DataFrame containing the results of the LIME explanation. For each model output, the table
    ...
    def map(self) -> dict[str, Saliency]:
        ...
        -------
        dict[str, Saliency]
             A dictionary keyed by output name, and the values will be the corresponding :class:`~trustyai.model.Saliency` object.
        """
...
class LimeExplainer:
    ...
    LIME (`Local Interpretable Model-agnostic Explanations <https://arxiv.org/abs/1602.04938>`_) seeks to answer this
    question via providing *saliencies*, weights associated with each input feature that describe how strongly said
    ...
    def __init__(
        ...
        penalise_sparse_balance : bool
            Whether to penalise features that are likely to produce linearly inseparable outputs. This can improve
            the efficacy and interpretability of the outputted saliencies.
        normalise_weights : bool
            Whether to normalise the saliencies generated by LIME. If selected, saliencies will be normalized between 0 and 1.
        """
    ...
    def explain(self, prediction, model: PredictionProvider) -> LimeResults:
        ...
        prediction : :obj:`~trustyai.model.Prediction`
            A :class:`~trustyai.model.Prediction` as returned by :func:`~trustyai.model.simple_prediction`. This object
            wraps the original model input and output together.
        model : :obj:`~trustyai.model.PredictionProvider`
            The TrustyAI PredictionProvider, as generated by :class:`~trustyai.model.Model` or :class:`~trustyai.model.ArrowModel`.
...
class SHAPResults(ExplanationVisualiser):
    ...
    def as_dataframe(self) -> pd.DataFrame:
        ...
        -------
        pandas.DataFrame
            DataFrame containing the results of the SHAP explanation. For each model output, the table
            will contain the following columns, indexed by feature name:
    ...
    def candlestick_plot(self) -> None:
        """Visualize the SHAP explanation of each output as a set of candlestick plots, one per output."""
        with mpl.rc_context(drcp):
            for i, saliency in enumerate(self.shap_results.getSaliencies()):
                shap_values = [
...
class SHAPExplainer:
    ...
    SHAP (`SHapley Additive exPlanations <https://arxiv.org/abs/1705.07874>`_) seeks to answer this
    question via providing SHAP values that provide an additive explanation of the model output; essentially
    a `receipt` for the model's output. SHAP does this by finding an *additive explanatory model* :math:`g` of the
    form:

    .. math::
        f(x) = \phi_0 + \phi_1 x'_1 + \phi_2 x'_2 + \dots + \phi_n x'_n

    where :math:`x'_1, \dots, x'_n` are binary values that indicate whether the :math:`n` th feature is present or absent
    and :math:`\phi_1, \dots, \phi_n` are those features' corresponding SHAP values..
    :math:`\phi_0` is the *fnull* of the model, indicating the model's latent
    output in the absence of all features; functionally, the y-intercept of the explanatory model.

    What all this means is that a feature's exact contribution to the output can be seen as its SHAP value, and the original
    model output can be recovered by summing up the fnull with all SHAP values.

    To operate, SHAP also needs access to a *background dataset*, a set of representative input datapoints that captures
    the model's "normal behavior". All SHAP values are implicitly comparisons against to the background data, i.e.,
    *By how much did each feature contribute to the outputs, as compared to the background inputs?*
    """
    ...
    def __init__(
        ...
        samples: int
            The number of samples to use when computing SHAP values. Higher values will increase explanation accuracy, at the
            cost of runtime.
        batch_size: int
            The number of batches passed to the PredictionProvider at once. When using a :class:`~Model` in the :func:`explain`
            function, this parameter has no effect. With an :class:`~ArrowModel`, `batch_sizes`
            of around :math:`\frac{2000}{\mathtt{len(background)}}` can produce significant performance gains.
        seed: int
        ...
        link_type : :obj:`~_ShapConfig.LinkType`
            ...
            or ``trustyai.explainers._ShapConfig.LinkType.LOGIT``. If the model output is a probability, choosing the ``LOGIT``
            link will rescale explanations into log-oods units. Otherwise, choose ``IDENTITY``.
