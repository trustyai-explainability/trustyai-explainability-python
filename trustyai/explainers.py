...
class SHAPExplainer:
    ...
    where :math:`x'_1, \dots, x'_n` are binary values that indicate whether the :math:`n` th feature is
    present or absentand :math:`\phi_1, \dots, \phi_n` are those features' corresponding SHAP values..
    ...
    What all this means is that a feature's exact contribution to the output can be seen as its SHAP value, and
    the original model output can be recovered by summing up the fnull with all SHAP values.
    ...
    *By how much did each feature contribute to the outputs, as compared to the background inputs?*
    ...
    def __init__(
        ...
        samples: int
            The number of samples to use when computing SHAP values. Higher values will increase explanation
             accuracy, at the  cost of runtime.
        batch_size: int
            The number of batches passed to the PredictionProvider at once. When using a :class:`~Model` in
            the :func:`explain` function, this parameter has no effect. With an :class:`~ArrowModel`, `batch_sizes`
        ...
        link_type : :obj:`~_ShapConfig.LinkType`
            ...
            or ``trustyai.explainers._ShapConfig.LinkType.LOGIT``. If the model output is a probability,
            choosing the ``LOGIT`` link will rescale explanations into log-oods units. Otherwise, choose ``IDENTITY``.
