...
def feature_domain(
    ...
    r"""Create a Java :class:`FeatureDomain`. This represents the valid range of values for a particular feature,
    which is useful when constraining a counterfactual explanation to ensure it only recovers valid inputs. For
    example, if we had a feature that described a person's age, we might want to constrain it to the range [0, 125]
    to ensure the counterfactual explanation doesn't return unlikely ages such as -5 or 715.
    ...
    -------
    :class:`FeatureDomain`
        A Java :class:`FeatureDomain` object, to be used in the :func:`~trustyai.model.feature` function.
